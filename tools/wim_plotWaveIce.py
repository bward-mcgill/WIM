...
def plotOneVar(REP_IN_CICE, fileCI, REP_IN_W3, fileW3, nlon, nlat, lat, lon, repOUT, case, datestr, plot_type, list_var):
    ...
    for var in list_var:
       data=read_data(REP_IN_CICE, [fileCI], 'fsdrad', nlon, nlat)
...
#       m = Basemap(projection='ortho',lon_0=270,lat_0=40,resolution='l')
       ...
       if plot_type == 'scatter':
           ...
           if plot_type == 'contour':
               ...
               #Plot contours
               if fileW3 != "none":
                   cont=axes.contour(x,y, d1hs, colors='black') #, levels=[0.02, 0.04, 0.06, 0.1, 0.14, 0.2])
    ...
    for ts in list_ts:
        ...
        if pp_prod == "hourly":
            if coupled == "true":
                ...
                #Creer un function qui decide c'est quoi les fichier qui sont choisi pour le post-processing. 
                print("Coupled")
...
#              fileW3="ww3.2005-01-01-07200.nc"
                ...
                if outfreqU == 's':
                    ...
                    fileCI="iceh."+datestrCI+".nc"
                fileW3="none"
     #           REP_IN_W3='~/wim/ww3/model/out/case43'
     #           fileW3='ww3.2005-01-01-03600.nc'
                datestrW3=datestrCI
