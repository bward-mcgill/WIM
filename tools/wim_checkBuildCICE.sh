...
rm -f ${WIM_REP_TOOLS}/testListMD5_CICE.txt

for file in $ci_list_src
do
   md5sum $file >> ${WIM_REP_TOOLS}/testListMD5_CICE.txt 
done
...
elif ! $(cmp -s "${WIM_REP_TOOLS}/listMD5_CICE.txt" "${WIM_REP_TOOLS}/testListMD5_CICE.txt"); then
   echo '|------------CICE source code has changed build again.-------------|'
   cd ${CI_REP_WRK}
   csh ${CI_REP_WRK}/cice.build
   cat ${WIM_REP_TOOLS}/testListMD5_CICE.txt > ${WIM_REP_TOOLS}/listMD5_CICE.txt
